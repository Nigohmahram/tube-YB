				...
				<Box width={{ xs: '100%', md: '75%' }}>
					...
					{videoDetail.snippet.tags.map((item, idx) => (
						<Chip
							label={item}
							key={idx}
							sx={{ marginTop: '10px', cursor: 'pointer', ml: '10px', background: 'white', color: 'black' }}
							deleteIcon={<Tag />}
							onDelete={() => {}}
							variant='outlined'
						/>
					))}
					<Typography variant='h5' fontWeight={'bold'} p={'2px'} color={'white'}>
						{videoDetail.snippet.title}
					</Typography>
					<Typography variant='subtitle2' p={'2px'} color={'white'} sx={{ opacity: '.7' }}>
						{renderHtml(videoDetail.snippet.description)}
					</Typography>
					<Stack direction={'row'} gap={'20px'} alignItems={'center'} py={'1px'} px={'2px'}>
						{' '}
						<Stack direction={'row'} alignItems={'center'} gap={'3px'}>
							<Visibility />
							{parseInt(videoDetail.statistics.viewCount).toLocaleString()} views
						</Stack>
						<Stack direction={'row'} alignItems={'center'} gap={'3px'} sx={{ opacity: 0.7 }}>
							<FavoriteOutlined />
					...
					<Stack direction={'row'} py={'1px'} px={'2px'}>
